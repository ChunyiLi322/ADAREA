...
def get_an_initializate_sample(org_data, word_candidate_list, word_candidate_index_list, embeddings, vocab,data_index):
    print("-----------------------------------------------初始对抗样本的构建----------------------------------------------------------------------------------")
    for word_candidate, word_candidate_index in zip(word_candidate_list, word_candidate_index_list):
        ...
        print ("初始化一个对抗样本的单词",word_candidate,"的同义词集合",word_synonym_list)
        word_synonym_list = word_prune(word_synonym_list, word_candidate)
        count_initializate_index = 0
        for word_synonym_line in word_synonym_list:
            count_initializate_index = count_initializate_index + 1
            if count_initializate_index == len(word_synonym_list)-1:
                org_data = get_an_optimization_adsample(org_data, word_candidate_index, word_synonym_line, data_index)
                break
        if word_candidate_index % 2 == 1:  
           part_sentence = org_data[data_index]['text1'].split(' ')
           part_sentence[word_candidate_index]='_'
           part_sentence_string = listToString(part_sentence)
           org_data[data_index]['text1'] = part_sentence_string            
    return  org_data 


########################################################根据同义词选候选序列################################################################################

def get_word_by_candidate_name(find_synonym_word, embeddings, vocab, top_k):
    ...
    for i in track(range(2), description='canculate similarity...'):
        time.sleep(1)
    ...
    console.print("前100个相似单词的index")
    # print(top_k_idx) 
    console.print("前100个相似单词的word")
    # print(top_k_word) 
...
def sentence_prune(test_file_path, test_sentence_index):  
    ...
    # d = dict()
    # d['first'] = word_candidate_list
    # d['second'] = word_candidate_index_list
    # return  d
...
def word_prune(input_list, refer_word):    

    nlp=StanfordCoreNLP(r'/home/lcy/adsample/NCR2Code/')
    
    # fin=open('traintest.txt','r',encoding='utf8')
    fin = listToString(input_list)
    console.print("合成字符串",refer_word + " " + fin)
    ...
    n_standard =  word_tag[refer_index]           
    m_standard =  word_dict[refer_index]

    print("单词字典",word_dict)
    print("单词标记",word_tag)    
    ...
    for m,n in zip(word_dict,word_tag):
        if n_standard == 'NN' or n_standard == 'NNS' or n_standard == 'NNP':
            if n == 'NN' or n == 'NNS' or n == 'NNP':
               same_word_tag.append(m)
        if n_standard== 'VB' or n_standard == 'VBD' or n_standard == 'VBG'  or n_standard == 'VBN' or n_standard == 'VBP' or n_standard == 'VBZ':
            if n == 'VB' or n == 'VBD' or n == 'VBG'  or n == 'VBN' or n == 'VBP' or n == 'VBZ':
               same_word_tag.append(m)
        if n_standard == 'JJS' or n_standard == 'JJR' or n_standard == 'JJ'  or n_standard == 'RB' or n_standard == 'RBS' or n_standard == 'RBR':
...
def get_an_optimization_adsample(prior_sentence, candidate_index, word_synonym,data_index):
    part_sentence = []
    console.print("新的形成的对抗文本修改的索引是",candidate_index)
    part_sentence = prior_sentence[data_index]['text1'].split(' ')
    part_sentence[candidate_index]=word_synonym
    part_sentence_string = listToString(part_sentence)
    prior_sentence[data_index]['text1'] = part_sentence_string
    print("新的形成的对抗文本是啥",part_sentence_string)
    # print("新的形成的batch对抗样本",prior_sentence)
